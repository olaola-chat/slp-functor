...
func (*slice) UniqueUint64Array(array []uint64) []uint64 {
	checkMap := make(map[uint64]int)
	ret := make([]uint64, 0, len(array))
	for _, v := range array {
		if _, ok := checkMap[v]; !ok {
			ret = append(ret, v)
			checkMap[v] = 1
...
// 过滤重复的元素
func (s *slice) RemoveReplicaSliceUInt32(slc []uint32) []uint32 {
	result := make([]uint32, 0)
	tempMap := make(map[uint32]bool, len(slc))
	for _, e := range slc {
		if !tempMap[e] {
			tempMap[e] = true
...
// RemoveReplicaSliceInt64 过滤重复的元素
func (s *slice) RemoveReplicaSliceInt64(slc []int64) []int64 {
	var result []int64
	tempMap := make(map[int64]bool, len(slc))
	for _, e := range slc {
		if !tempMap[e] {
			tempMap[e] = true
			result = append(result, e)
